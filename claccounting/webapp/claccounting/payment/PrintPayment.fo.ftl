        ...
        <fo:table table-layout="fixed" width="100%">
          ...
          <fo:table-body>
            <fo:table-row>
              <fo:table-cell/>
              <fo:table-cell>
                <fo:block padding-before="2.2cm">
                 Nro:${payment.paymentRefNum?default("------")}
                 (${payment.paymentId})                 
                ${payment.effectiveDate?date?string.short}</fo:block>
              </fo:table-cell>
            </fo:table-row>
            <fo:table-row>
              <fo:table-cell padding-before="0.8cm">
                <fo:block margin-left="3.0cm">
                  <#assign toPartyNameResult = dispatcher.runSync("getPartyNameForDate", Static["org.ofbiz.base.util.UtilMisc"].toMap("partyId", payment.partyIdFrom, "compareDate", payment.effectiveDate, "userLogin", userLogin))/>
            ...
            <fo:table-row>
              ...
              <#--assign amount = Static["org.ofbiz.base.util.UtilNumber"].formatRuleBasedAmount(payment.getDouble("amount"), "%dollars-and-hundredths", locale).toUpperCase()-->
                <#assign amount = Static["org.ofbiz.base.util.UtilNumber"].formatRuleBasedAmount(payment.getDouble("amount"), "%guaranies", locale).toUpperCase()>
                <fo:block padding-before="0.4cm" margin-left="1.3cm">${amount}<#list 1..(100-amount.length()) as x>*</#list></fo:block>
              </fo:table-cell>
            </fo:table-row>
           <fo:table-row>
              <fo:table-cell number-columns-spanned="2">
                <fo:block padding-before="0.4cm" margin-left="1.3cm">(${payment.comments?default("Sin Referencias")})</fo:block>
              </fo:table-cell>
            </fo:table-row>
          </fo:table-body>
        </fo:table>
